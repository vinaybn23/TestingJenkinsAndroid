...
public class AplThumbImage implements Runnable {
    ...
    public AplThumbImage() {
    }

    public AplThumbImage(int threadNo, Context context, String upc, String pimhost, Handler handler) {
    ...
    private Bitmap getBitmap(String upcno) {
        ...
        try {
            ...
            OcutagAuthScheme.Builder builder = new OcutagAuthScheme.Builder("GET", String.valueOf(urlthumb), sendDateImage(date), AplMainFragment.authHeaders);
            OcutagAuthScheme authScheme = null;

            Log.d(TAG, "Auth Headers: " + Arrays.toString(AplMainFragment.authHeaders));
            try {
            ...
            if (rspCode == 200) {
                ...
                Log.d(TAG, "thumbhash size: " + thumbHash.size() + "  productlist size:" + AplMainFragment.aplFilteredArray.size());
        ...
        try {
            if (thumbHash.size() > 0) {
                ...
                sendMessage(AplMainFragment.UPDATE_ADAPTER, "update list");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
