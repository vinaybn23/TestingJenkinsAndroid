...
/**
 * Created by bharath.km on 08-03-2018.
 */
...
public class ZoomView extends RelativeLayout {
    ...
    public void zoomTo(final float zoom, final float x, final float y) {
        ...
        zoomX = x;
        zoomY = y;
        smoothZoomTo(this.zoom, x, y,true);
    }

    public void smoothZoomTo(final float zoom, final float x, final float y,Boolean isfromSameClass) {
        smoothZoom = clamp(1.0f, zoom, maxZoom);
        smoothZoomX = x;
        smoothZoomY = y;
        if (listener != null && isfromSameClass == true) {
    ...
    public boolean dispatchTouchEvent(final MotionEvent ev) {
        ...
        if (ev.getPointerCount() == 1) {
            ...
            Log.i("test", "dispatchTouchEvent: single tap ");
        ...
        if (ev.getPointerCount() == 2) {
            ...
            Log.i("test", "dispatchTouchEvent: double tap ");
        }
    ...
    private void processSingleTouchOnMinimap(final MotionEvent ev) {
        ...
        smoothZoomTo(smoothZoom, zx, zy,true);
    }
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (scrolling || (smoothZoom > 1.0f && l > 30.0f)) {
                    ...
                    smoothZoomY -= dy / zoom;
                    if (listener != null) {
                        listener.onZoomEnded(smoothZoom, smoothZoomX, smoothZoomY);
                    }
                    return;
                }
                break;
